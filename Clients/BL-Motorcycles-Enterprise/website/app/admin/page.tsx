...
const channelBadge = (ch?: string) => {
  if (ch?.toUpperCase() === "EBAY") return <span className="px-2 py-0.5 text-[8px] font-black uppercase tracking-wider bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 rounded-full">eBay</span>;
  return <span className="px-2 py-0.5 text-[8px] font-black uppercase tracking-wider bg-blue-500/10 text-blue-500 border border-blue-500/20 rounded-full">Website</span>;
};
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[200] flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[#0a0a0c] border border-white/10 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="p-6 border-b border-white/5">
          ...
          <p className="text-[10px] text-zinc-500 mt-2">{order.customer_name} — {order.ebay_order_id}</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-xs"><span className="text-zinc-500">Channel</span>{channelBadge(order.channel)}</div>
...
function AdminDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  const filtered = useMemo(() => {
    ...
    else if (orderFilter === "ebay") f = f.filter(o => o.channel?.toUpperCase() === "EBAY");
    else if (orderFilter === "website") f = f.filter(o => o.channel?.toUpperCase() !== "EBAY");
    ...
    if (search) {
      ...
      f = f.filter(o =>
        ...
        o.ebay_order_id?.toLowerCase().includes(s) ||
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                { id: "ebay", label: "eBay", count: orders.filter(o => o.channel?.toUpperCase() === "EBAY").length },
                ...
                <table className="w-full text-left border-collapse min-w-[900px]">
                  <thead>
                    <tr className="border-b border-white/5">
                      {["","Order / SKU","Customer","Channel","Sale","Trade","Margin","Status","Age"].map(h => (
                  ...
                  <tbody>
                    {loading ? (
                      <tr><td colSpan={9} className="p-8 text-center text-zinc-500 text-xs font-mono animate-pulse">Loading orders...</td></tr>
                    ) : filtered.length === 0 ? (
                      <tr><td colSpan={9} className="p-8 text-center text-zinc-500 text-xs font-mono">No orders match your filters</td></tr>
                      ...
                      return (
                        <>
                          <tr key={o.id} onClick={() => setExpandedOrder(expanded ? null : o.id)}
                            ...
                            <td className="p-3">
                              <div className="text-[11px] font-mono text-orange-400">{o.ebay_order_id?.substring(0, 20)}{(o.ebay_order_id?.length || 0) > 20 ? "..." : ""}</div>
                            ...
                            <td className="p-3">{channelBadge(o.channel)}</td>
