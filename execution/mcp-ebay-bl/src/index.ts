...
const server = new Server(
  { name: "mcp-ebay-bl", version: "2.0.0" },
  { capabilities: { tools: {} } }
);
...
async function getUserToken(): Promise<string> {
  if (cachedUserToken && Date.now() < tokenExpiry) return cachedUserToken;
  if (!EBAY_REFRESH_TOKEN) throw new Error("No EBAY_REFRESH_TOKEN — cannot access seller APIs");
  ...
  const res = await axios.post(
    `${BASE_URL}/identity/v1/oauth2/token`,
    `grant_type=refresh_token&refresh_token=${encodeURIComponent(EBAY_REFRESH_TOKEN)}&scope=${encodeURIComponent(scopes)}`,
    { headers: { "Content-Type": "application/x-www-form-urlencoded", Authorization: `Basic ${auth}` } }
  );
...
function sellerHeaders(token: string) {
  return {
  ...
  try {
    ...
    if (name === "search_listings") {
      ...
      const res = await axios.get(
        `${BASE_URL}/buy/browse/v1/item_summary/search?q=${encodeURIComponent(q)}&limit=${limit}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const items = res.data.itemSummaries?.map((i: any) => ({
    ...
    if (name === "get_inventory_items") {
      ...
      const res = await axios.get(
        `${BASE_URL}/sell/inventory/v1/inventory_item?limit=${limit}&offset=${offset}`,
        { headers: sellerHeaders(token) }
      );
      const items = res.data.inventoryItems?.map((i: any) => ({
        sku: i.sku,
    ...
    if (name === "get_offers") {
      ...
      const token = await getUserToken();
      const res = await axios.get(
        `${BASE_URL}/sell/inventory/v1/offer?sku=${encodeURIComponent(sku)}&limit=100`,
        { headers: sellerHeaders(token) }
      );
      return { content: [{ type: "text", text: JSON.stringify(res.data, null, 2) }] };
    }
    ...
    if (name === "withdraw_offer") {
      ...
      const token = await getUserToken();
      const res = await axios.post(
        `${BASE_URL}/sell/inventory/v1/offer/${offerId}/withdraw`,
        {},
        { headers: sellerHeaders(token) }
      );
    ...
    if (name === "update_inventory_item") {
      ...
      if (price !== undefined) {
        const offersRes = await axios.get(
          `${BASE_URL}/sell/inventory/v1/offer?sku=${encodeURIComponent(sku)}&limit=10`,
          { headers: sellerHeaders(token) }
        );
        for (const offer of offersRes.data.offers || []) {
          ...
          await axios.put(
            `${BASE_URL}/sell/inventory/v1/offer/${offer.offerId}`,
            offer,
            { headers: sellerHeaders(token) }
          );
        }
    ...
    if (name === "get_order") {
      ...
      const token = await getUserToken();
      const res = await axios.get(
        `${BASE_URL}/sell/fulfillment/v1/order/${orderId}`,
        { headers: sellerHeaders(token) }
      );
      return { content: [{ type: "text", text: JSON.stringify(res.data, null, 2) }] };
    }
    ...
    if (name === "bulk_withdraw_offers") {
      ...
      const token = await getUserToken();
      const results: { sku: string; status: string; offersWithdrawn: number }[] = [];
      ...
      for (const sku of skus) {
        try {
          ...
          for (const offer of offersRes.data.offers || []) {
            try {
              await axios.post(
                `${BASE_URL}/sell/inventory/v1/offer/${offer.offerId}/withdraw`,
                {},
                { headers: sellerHeaders(token) }
              );
              withdrawn++;
    ...
    if (name === "get_active_listing_count") {
      const token = await getUserToken();
      const res = await axios.get(
        `${BASE_URL}/sell/inventory/v1/inventory_item?limit=1&offset=0`,
        { headers: sellerHeaders(token) }
      );
