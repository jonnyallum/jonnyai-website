...
def run_imports():
    ...
    cwd = 'c:/Users/jonny/Desktop/JonnyAI_JaiOS_4.0/infra/terraform/foundation'
    
    def tf_import(addr, id):
        print(f"Importing {addr} -> {id}...")
        cmd = ['terraform', 'import', addr, id]
        try:
            subprocess.run(cmd, cwd=cwd, env=env, check=True, capture_output=True, text=True)
        except subprocess.CalledProcessError as e:
            if "already exists" in e.stderr or "already imported" in e.stderr:
                print("  Skipping (already in state)")
            else:
    ...
    for f in folders:
        ...
        if display_name == 'Common':
            tf_import('module.cs-common.google_folder.folders["Common"]', f'folders/{fid}')
        
        if display_name in ['Department 1', 'Department 2', 'Department 3']:
            tf_import(f'module.cs-folders-level-0["{display_name}"].google_folder.folders["{display_name}"]', f'folders/{fid}')
            ...
            for sf in r_sub.json().get('folders', []):
                ...
                tf_import(f'module.cs-folders-level-1["{path_l1}"].google_folder.folders["{s_display_name}"]', f'folders/{sfid}')
                ...
                for ssf in r_sub2.json().get('folders', []):
                    ...
                    tf_import(f'module.cs-folders-level-2["{path_l2}"].google_folder.folders["{ss_display_name}"]', f'folders/{ssfid}')
