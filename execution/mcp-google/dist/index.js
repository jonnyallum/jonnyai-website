...
function getAuthedClient() {
    ...
    if (!tokens)
        throw new Error("Not authenticated. Call google_auth_url, open the URL, authorize, then call google_auth_exchange with the code from the redirect URL.");
...
async function geminiGenerate(prompt, model = "gemini-2.0-flash") {
    const m = genAI.getGenerativeModel({ model });
    const result = await m.generateContent(prompt);
    return result.response.text();
}
async function geminiChat(history, message, model = "gemini-2.0-flash") {
    const m = genAI.getGenerativeModel({ model });
...
async function driveReadFile(fileId) {
    ...
    if (mimeType === "application/vnd.google-apps.document") {
        ...
        return { name: meta.data.name, content: ex.data };
    }
...
async function driveSearch(query, maxResults = 20) {
    const drive = google.drive({ version: "v3", auth: getAuthedClient() });
...
async function driveUploadText(name, content, parentId) {
    const drive = google.drive({ version: "v3", auth: getAuthedClient() });
...
async function gmailSend(to, subject, body, htmlBody) {
    const gmail = google.gmail({ version: "v1", auth: getAuthedClient() });
...
// ── Calendar ──────────────────────────────────────────────────────────────────
async function calendarListEvents(calendarId = "primary", maxResults = 20, timeMin, timeMax) {
...
async function calendarCreateEvent(summary, start, end, description, attendees, calendarId = "primary") {
    const cal = google.calendar({ version: "v3", auth: getAuthedClient() });
...
async function calendarUpdateEvent(eventId, updates, calendarId = "primary") {
    const cal = google.calendar({ version: "v3", auth: getAuthedClient() });
    const res = await cal.events.patch({ calendarId, eventId, requestBody: updates });
    return res.data;
}
async function calendarDeleteEvent(eventId, calendarId = "primary") {
    const cal = google.calendar({ version: "v3", auth: getAuthedClient() });
...
async function docsRead(documentId) {
    const docs = google.docs({ version: "v1", auth: getAuthedClient() });
...
async function sheetsRead(spreadsheetId, range = "Sheet1") {
    const sheets = google.sheets({ version: "v4", auth: getAuthedClient() });
...
async function tasksCreate(title, notes, due, tasklistId = "@default") {
    const tasks = google.tasks({ version: "v1", auth: getAuthedClient() });
    const res = await tasks.tasks.insert({ tasklist: tasklistId, requestBody: { title, notes, due } });
    return res.data;
}
async function tasksComplete(taskId, tasklistId = "@default") {
    const tasks = google.tasks({ version: "v1", auth: getAuthedClient() });
...
async function mapsGeocode(address) {
...
async function mapsPlacesSearch(query, location, radius = 5000) {
...
async function mapsDirections(origin, destination, mode = "driving") {
...
async function mapsDistanceMatrix(origins, destinations, mode = "driving") {
    ...
    return {
        ...
        rows: data.rows.map((row) => row.elements.map((el) => ({
            status: el.status,
...
async function analyticsRunReport(propertyId, metrics, dimensions, startDate = "7daysAgo", endDate = "today") {
    ...
    return {
        ...
        headers: {
            dimensions: res.data.dimensionHeaders?.map((h) => h.name),
            metrics: res.data.metricHeaders?.map((h) => h.name),
        },
        rows: rows.map((r) => ({
...
async function analyticsRealtimeReport(propertyId, metrics, dimensions) {
    const prop = propertyId.startsWith("properties/") ? propertyId : `properties/${propertyId}`;
    const analyticsData = google.analyticsdata({ version: "v1beta", auth: getAuthedClient() });
    ...
    try {
        ...
        switch (name) {
            ...
            case "gemini_generate":
                result = { text: await geminiGenerate(a.prompt, a.model) };
                break;
            case "gemini_chat":
                result = { text: await geminiChat(a.history ?? [], a.message, a.model) };
                break;
            // Drive
            case "drive_list_files":
                result = await driveListFiles(a.query, a.maxResults);
                break;
            case "drive_search":
                result = await driveSearch(a.query, a.maxResults);
                break;
            case "drive_read_file":
                result = await driveReadFile(a.fileId);
                break;
            case "drive_upload_text":
                result = await driveUploadText(a.name, a.content, a.parentId);
                break;
            case "drive_create_folder":
                result = await driveCreateFolder(a.name, a.parentId);
                break;
            // Gmail
            case "gmail_list":
                result = await gmailList(a.query, a.maxResults);
                break;
            case "gmail_read":
                result = await gmailRead(a.messageId);
                break;
            case "gmail_send":
                result = await gmailSend(a.to, a.subject, a.body, a.htmlBody);
                break;
            // Calendar
            case "calendar_list_events":
                result = await calendarListEvents(a.calendarId, a.maxResults, a.timeMin, a.timeMax);
                break;
            case "calendar_list_calendars":
                result = await calendarListCalendars();
                break;
            case "calendar_create_event":
                result = await calendarCreateEvent(a.summary, a.start, a.end, a.description, a.attendees, a.calendarId);
                break;
            case "calendar_update_event":
                result = await calendarUpdateEvent(a.eventId, a.updates, a.calendarId);
                break;
            case "calendar_delete_event":
                result = await calendarDeleteEvent(a.eventId, a.calendarId);
                break;
            // Docs
            case "docs_create":
                result = await docsCreate(a.title, a.content);
                break;
            case "docs_read":
                result = await docsRead(a.documentId);
                break;
            case "docs_append":
                result = await docsAppend(a.documentId, a.text);
                break;
            // Sheets
            case "sheets_create":
                result = await sheetsCreate(a.title);
                break;
            case "sheets_read":
                result = await sheetsRead(a.spreadsheetId, a.range);
                break;
            case "sheets_write":
                result = await sheetsWrite(a.spreadsheetId, a.range, a.values);
                break;
            case "sheets_append":
                result = await sheetsAppend(a.spreadsheetId, a.range, a.values);
                break;
            // Tasks
            case "tasks_list_lists":
                result = await tasksListLists();
                break;
            case "tasks_list":
                result = await tasksList(a.tasklistId, a.showCompleted);
                break;
            case "tasks_create":
                result = await tasksCreate(a.title, a.notes, a.due, a.tasklistId);
                break;
            case "tasks_complete":
                result = await tasksComplete(a.taskId, a.tasklistId);
                break;
            // Contacts
            case "contacts_list":
                result = await contactsList(a.maxResults);
                break;
            case "contacts_search":
                result = await contactsSearch(a.query);
                break;
            // Maps
            case "maps_geocode":
                result = await mapsGeocode(a.address);
                break;
            case "maps_reverse_geocode":
                result = await mapsReverseGeocode(a.lat, a.lng);
                break;
            case "maps_places_search":
                result = await mapsPlacesSearch(a.query, a.location, a.radius);
                break;
            case "maps_directions":
                result = await mapsDirections(a.origin, a.destination, a.mode);
                break;
            case "maps_distance_matrix":
                result = await mapsDistanceMatrix(a.origins, a.destinations, a.mode);
                break;
            // Analytics
            case "analytics_list_properties":
                result = await analyticsListProperties();
                break;
            case "analytics_run_report":
                result = await analyticsRunReport(a.propertyId, a.metrics, a.dimensions, a.startDate, a.endDate);
                break;
            case "analytics_realtime":
                result = await analyticsRealtimeReport(a.propertyId, a.metrics, a.dimensions);
                break;
