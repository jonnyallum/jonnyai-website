...
def create_ebay_listing(product, access_token: str):
    ...
    brand = product.get('brand') or 'Lextek'
    sku = product.get('sku', '')
    
    # Enrich description with fitment/compatibility data if present
    description = f"<h1>{product['title']}</h1>"
    if product.get('description'):
        description += f"<div style='margin-bottom:20px;'>{product['description'].replace(chr(10), '<br>')}</div>"
    description += f"<p>Genuine {brand} Part. High quality replacement/upgrade.</p><p>SKU: {sku}</p>"

    qty = min(product.get('stock_level') or 1, 5)
    price = product.get('selling_price') or product.get('retail_price') or 9.99
    
    # Handle Image URL encoding
...
def main():
    print("=== BL Motorcycles — eBay Sync (Lextek / CMPO) ===")

    # Get fresh OAuth token
    access_token = get_oauth_token()

    # Fetch Llexeter products that are active and in stock
    res = supabase.table("products").select("*").eq("supplier_id", "LLEXETER").eq("is_active_on_ebay", True).gt("stock_level", 0).limit(50).execute()
    products = res.data
    print(f"Found {len(products)} active Lextek/CMPO products to sync.")

    for i, p in enumerate(products):
        print(f"[{i+1}/{len(products)}] {p['sku']} - {p['title'][:40]}...", end=" ", flush=True)
        result = create_ebay_listing(p, access_token)

        if result['success']:
            print(f"OK -> ItemID {result['item_id']}")
            # Save ItemID back to Supabase
            supabase.table("products").update({"ebay_item_id": result['item_id']}).eq("id", p["id"]).execute()
        else:
