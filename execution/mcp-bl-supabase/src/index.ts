  ...
  try {
    switch (name) {
      case "upsert_products": {
        const { products } = args as any;
        const { data, error } = await supabase
          .from("products")
          .upsert(products, { onConflict: "sku" })
          .select();

        if (error) throw new McpError(ErrorCode.InternalError, `Supabase Error: ${error.message}`);
        return { content: [{ type: "text", text: `Successfully upserted ${data?.length} products.` }] };
      }

      case "upsert_fitment": {
        const { mappings } = args as any;
        const { data, error } = await supabase
          .from("fitment")
          .upsert(mappings)
          .select();
      ...
      case "query_products": {
        const { query, supplier_id, min_price, max_price, limit } = args as any;
        let q = supabase.from("products").select("*");

        if (query) q = q.ilike("title", `%${query}%`);
        if (supplier_id) q = q.eq("supplier_id", supplier_id);
        if (min_price) q = q.gte("selling_price", min_price);
        if (max_price) q = q.lte("selling_price", max_price);
